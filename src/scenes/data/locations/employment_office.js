			...
			if(typeof scene.locations[i].jobs !="undefined") {
				employers.push({
					...
					"use": function(scene,item,location) {
						...
						var jy = 6;
						job_listing.add(new Phaser.GameObjects.BitmapText(scene,91,jy,"small",item.name+" Jobs Available:").setOrigin(0.5));
						var job_location = scene.get_location(item.id);
						var jobs = job_location.jobs;
						jy+=8;
						for(var i in jobs) {
							var job_name = new Phaser.GameObjects.BitmapText(scene,0,0,"small",jobs[i].name).setOrigin(0);
							var wage = Math.max(Math.round(jobs[i].wage*scene.gamestate.economy),scene.settings.minimum_wage);
							var job_pay = new Phaser.GameObjects.BitmapText(scene,0,0,"small","$"+wage+" Hr.").setOrigin(0);
							...
							job_clicker = new Phaser.GameObjects.Rectangle(scene,0,3,job_pay.x+job_pay.width,8).setOrigin(0,0)
								...
								.on("pointerup",function() {
									...
									if(scene.player.dependability<job.dependability) {
										got_job = false;
						...
						//this goes through every object in the location_window's container, and looks to 
						//see if its texture is "btn-done". If it is, it destroys it and creates a new button in its place.
						for(var i in scene.location_window.list) {
							if((typeof scene.location_window.list[i].texture!="undefined") && (scene.location_window.list[i].texture.key=="btn-done")) {
								var btn_x = scene.location_window.list[i].x;
								scene.location_window.list[i].destroy();
							}
						}
